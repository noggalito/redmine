...
#    end
  ...
  def to_csv
    ...
    FCSV.generate(out, :encoding => 'cp1252', :force_quotes => true, :col_sep => ";", :headers => true) do |csv|
      csv << csv_header
  ...
  def csv_header
    csv_data = [
      '#',
      l(:label_date).encode!("iso-8859-1"),
      l(:label_member).encode!("iso-8859-1"),
      l(:label_activity).encode!("iso-8859-1"),
      l(:label_project).encode!("iso-8859-1"),
      l(:label_version).encode!("iso-8859-1"),
      l(:label_issue).encode!("UTF-8", "iso-8859-1"),
      "#{l(:label_issue)} #{l(:field_subject)}".encode!("iso-8859-1"),
      l(:field_comments).encode!("iso-8859-1"),
      l(:field_hours).encode!("iso-8859-1"),
      l(:field_cost).encode!("iso-8859-1"),
      l(:field_total_cost).encode!("iso-8859-1")
    ]
  ...
  def time_entry_to_csv(time_entry)
    csv_data = [
      ...
      time_entry.comments,
      time_entry.hours.to_s.gsub('.',','),
      time_entry.user.custom_value_for(Setting.plugin_redmine_timesheet_plugin['cost_id']).value.gsub('.',','),
      (time_entry.hours.to_f * time_entry.user.custom_value_for(Setting.plugin_redmine_timesheet_plugin['cost_id']).value.to_f).round(2).to_s.gsub('.',',')
    ]
