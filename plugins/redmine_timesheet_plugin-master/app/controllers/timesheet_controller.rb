...
#  verify :method => :delete, :only => :reset, :render => {:nothing => true, :status => :method_not_allowed }
  ...
  def report
    ...
    # @user_cost_logs = UserCostLog.where(:user_id => get_user_id)

    if params && params[:timesheet]
      @timesheet = Timesheet.new(params[:timesheet])

      # (@timesheet.users).each do |user_id_cost_logs|
      #   puts user_id_cost_logs
      #   @user_cost_logs = UserCostLog.where(:user_id => user_id_cost_logs)
      #   puts @user_cost_logs.blank?
      # end
  ...
  def get_cost_id
    @cost_id = Setting.plugin_redmine_timesheet_plugin['cost_id']
  end
